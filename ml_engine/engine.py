...
class HealthcareMLEngine:
    ...
    #         'feature_cols': cols,
    #     }
    def run_clustering(self, feature_cols, n_clusters=3, model_type='kmeans'):
        X, _, cols = self.preprocess(feature_cols)
        X_scaled = self.scaler.fit_transform(X)

        if model_type == 'dbscan':
                from sklearn.cluster import DBSCAN
                # Auto-tune eps using nearest-neighbor distances
                from sklearn.neighbors import NearestNeighbors
                ...
                eps = max(eps, 0.3)
                db = DBSCAN(eps=eps, min_samples=max(3, len(X)//20))
                labels = db.fit_predict(X_scaled)
                n_found = len(set(labels)) - (1 if -1 in labels else 0)
                n_clusters_used = max(n_found, 1)
                algo_name = 'DBSCAN'
                algo_params = {'eps': round(eps, 3), 'min_samples': max(3, len(X)//20)}
        elif model_type == 'hierarchical':
                from sklearn.cluster import AgglomerativeClustering
                hc = AgglomerativeClustering(n_clusters=n_clusters, linkage='ward')
                labels = hc.fit_predict(X_scaled)
                n_clusters_used = n_clusters
                algo_name = 'Hierarchical (Ward)'
                algo_params = {'n_clusters': n_clusters, 'linkage': 'ward'}
        else:
                # Default: K-Means
                km = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
                labels = km.fit_predict(X_scaled)
        ...
        scatter = [{'x': round(float(coords[i, 0]), 3),
            ...
            # cluster_distribution for the doughnut chart (exclude noise)
        cluster_distribution = [
                {'cluster': f'Cluster {s["cluster"]}', 'count': s['size']}
                for s in cluster_stats
            ]
        if n_noise > 0:
                cluster_distribution.append({'cluster': 'Noise', 'count': n_noise})
