...
class DataSession(models.Model):
    ...
    columns_json   = models.TextField(default='[]')   # JSON list of column info dicts
    row_count      = models.IntegerField(default=0)
    col_count      = models.IntegerField(default=0)
    created_at     = models.DateTimeField(auto_now_add=True)
    preview_json   = models.TextField(default='[]')   # first 500 rows for preview table
    data_json      = models.TextField(default='[]')   # full dataset rows as JSON (for ML)
    ...
    def get_dataframe(self):
        """Reconstruct pandas DataFrame from stored JSON."""
        import pandas as pd
        rows = json.loads(self.data_json)
        if not rows:
            return None
