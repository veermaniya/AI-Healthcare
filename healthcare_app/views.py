...
def JR(data, status=200):
    """
    Drop-in JsonResponse replacement that handles numpy types.
    Use this instead of JsonResponse() everywhere in this file.
    """
    return HttpResponse(
...
def _engine(request):
    df = _df(request)
    if df is None:
        return None, JR({'success': False, 'error': 'No dataset loaded. Please upload a file first.'})
...
def api_login(request):
    ...
    try:
        ...
        u = authenticate(request, username=data.get('username',''), password=data.get('password',''))
        if u:
            login(request, u)
            return JR({'success': True})
        return JR({'success': False, 'error': 'Invalid username or password'})
...
def upload_file(request):
    ...
    try:
        ...
        if not file:
            return JR({'success': False, 'error': 'No file provided'})
        ...
        return JR({'success': True, 'file_name': file.name,
...
def upload_temp(request):
    ...
    try:
        file = request.FILES.get('file')
        if not file:
            return JR({'success': False, 'error': 'No file'})
        df = load_excel_file(file)
        summary = dataframe_summary(df)
...
def connect_sql(request):
    ...
    try:
        ...
        return JR({'success': True, 'file_name': 'SQL Query',
                   'rows': summary['rows'], 'columns': summary['columns'],
                   'column_info': col_info, 'preview': summary['preview']})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def run_analysis(request):
    ...
    try:
        ...
        df = _df(request)
        if df is None:
            return JR({'success': False, 'error': 'No dataset loaded'})
        if not feats:
            return JR({'success': False, 'error': 'Select at least one feature column'})
        e = HealthcareMLEngine(); e.load_dataframe(df)
        if task == 'auto' and tgt:
            task = e.auto_detect_task(tgt)
        if task == 'clustering' or not tgt:
            # result = e.run_clustering(feats, n_clusters=k)
            result = e.run_clustering(feats, n_clusters=k, model_type=model)
        ...
        if db_s:
            AnalysisResult.objects.create(session=db_s, task_type=task,
                ...
                model_type=model, result_json=json.dumps(result, cls=NumpyEncoder))
        return JR({'success': True, 'result': result})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def predict_input(request):
    ...
    try:
        ...
        return JR({'success': True, 'prediction': result})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def suggest_alert_rules(request):
    try:
        ...
        return JR({'success': True, 'rules': rules})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def compare_datasets(request):
    ...
    try:
        ...
        if df_a is None or df_b is None:
            return JR({'success': False, 'error': 'Upload both datasets first'})
...
def clinical_insights(request):
    ...
    try:
        ...
        return JR({'success': True, 'insights': result})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def anonymise(request):
    ...
    try:
        ...
        return JR({'success': True, 'masked_columns': summary,
...
def clinical_coding(request):
    try:
        ...
        return JR({'success': True, 'icd_codes': icd,
...
def chat(request):
    ...
    try:
        ...
        if not msg:
            return JR({'success': False, 'error': 'Empty message'})
        ...
        return JR({'success': True, 'message': resp['message'],
...
def analyze_image(request):
    ...
    try:
        ...
        if not key or key == 'your-groq-api-key-here':
            return JR({'success': False, 'error': 'Groq API key not configured'})
        ...
        return JR({'success': False, 'error': f'All vision models failed. {last_err}'})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def get_columns(request):
    ...
    db_s = DataSession.objects.filter(session_key=sk).first()
    if not db_s:
        return JR({'success': False, 'columns': []})
    return JR({'success': True, 'columns': db_s.get_columns(),
...
def get_column_stats_view(request):
    df = _df(request)
    if df is None:
        return JR({'success': False, 'stats': {}})
    try:
        return JR({'success': True, 'stats': get_column_stats(df)})
    except Exception as ex:
        return JR({'success': False, 'stats': {}, 'error': str(ex)})
...
def filter_data(request):
    ...
    try:
        ...
        return JR({'success': True, 'rows': len(df_f), 'preview': preview, 'filtered': True})
    except Exception as ex:
        return JR({'success': False, 'error': str(ex)})
...
def test_llm(request):
    key = settings.GROQ_API_KEY
    if not key or key == 'your-groq-api-key-here':
        return JR({'api_key_set': False, 'message': 'KEY NOT SET'})
    llm = GroqLLMClient(api_key=key)
    r   = llm.ask("Say hello in one sentence.")
    return JR({'api_key_set': True, 'api_key_preview': key[:8]+'...'+key[-4:],
...
def debug_key(request):
    ...
    return JR({'key_preview': (key[:8]+'...'+key[-4:]) if key and len(key)>12 else key or 'EMPTY',
